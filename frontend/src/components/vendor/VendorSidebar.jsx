...
const QUICK_ROLES = [
  { role: 'Vendor', email: 'vendor@apex.internal', pass: 'vendor123', name: 'Vikram Malhotra (Apex Global)' },
  { role: 'Lead Procurement Officer', email: 'admin@procureiq.internal', pass: 'admin123', name: 'Priya Sharma (Gov Portal)' },
  { role: 'Plant Head', email: 'planthead@procureiq.internal', pass: 'plant123', name: 'Rajesh Verma (Gov Portal)' },
  { role: 'VP Operations', email: 'vpops@procureiq.internal', pass: 'vp123', name: 'Kavita Reddy (Gov Portal)' },
  { role: 'Finance Director', email: 'finance@procureiq.internal', pass: 'finance123', name: 'Arjun Patel (Gov Portal)' },
];
...
export default function VendorSidebar({
  ...
  const navItems = [
    ...
    {
      id: 'vendor_negotiations',
      label: 'Negotiations',
      icon: MessageSquare,
    },
    {
      id: 'vendor_awarded_orders',
      ...
      <div className="flex flex-col h-full overflow-y-auto">
        ...
        <div className="p-4 border-b border-[#e8e6df] flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-md bg-purple-600 text-white flex items-center justify-center shadow-sm">
              <Store className="w-4 h-4" />
            </div>
            <div>
              <div className="flex items-center gap-1.5 leading-none">
                <span className="font-bold text-sm text-slate-900 tracking-tight">LokProcure</span>
                <span className="text-[9px] uppercase font-mono tracking-wider px-1.5 py-0.5 rounded bg-purple-50 text-purple-700 border border-purple-200 font-semibold">
                  VENDOR
                </span>
              </div>
              <span className="text-[10px] text-slate-500 font-mono tracking-tight">Supplier Marketplace</span>
            </div>
          </div>
        </div>

        {/* Vendor Persona Switcher */}
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 min-w-0">
                <div className="w-6 h-6 rounded bg-purple-100 border border-purple-200 flex items-center justify-center text-purple-700 font-bold text-[10px] shrink-0">
                  {user?.full_name?.split(' ').map(n => n[0]).join('') || 'V'}
                </div>
                <div className="overflow-hidden min-w-0">
                  <div className="font-semibold text-xs text-slate-900 truncate">{user?.full_name || 'Vikram Malhotra'}</div>
                  <div className="text-[10px] text-slate-500 truncate flex items-center gap-1">
                    <Building className="w-2.5 h-2.5 text-slate-400" /> {user?.department || 'Apex Global Industrial'}
                  </div>
                </div>
              </div>
            ...
            <div className="mt-2">
              <span className="inline-block text-[9px] font-mono uppercase tracking-wider px-1.5 py-0.2 rounded bg-white border border-[#e8e6df] text-purple-700 font-medium">
                Verified Supplier
              </span>
            </div>
          </button>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.badge && (
                  <span className="px-1.5 py-0.2 text-[10px] font-mono font-semibold rounded bg-purple-100 text-purple-800 border border-purple-200">
                    {item.badge}
                  </span>
                )}
              </button>
      ...
      <div className="p-3 border-t border-[#e8e6df] space-y-2 bg-[#f5f4f0]">
        <div className="px-2.5 py-1.5 rounded bg-[#fbfbfa] border border-[#e8e6df] flex items-center justify-between text-xs">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-slate-600 font-mono">Marketplace</span>
          </div>
